...
public class RetryHelper {
     ...
     */
    private final SecretManagerProperties.Retry config;

    public RetryHelper(SecretManagerProperties.Retry config) {
        this.config = config;
    }
            ...
            try {
                ...
                if (attempt >= config.getMaxAttempts()) {
                    log.warn("Max retry attempts ({}) exhausted for operation: {}",
                            config.getMaxAttempts(), e.getMessage());
                    throw e;
                }
                ...
                log.debug("Retryable exception on attempt {}/{}, retrying after {}ms: {}",
                        attempt, config.getMaxAttempts(), delay, e.getMessage());
                ...
                delay = Math.min(
                        (long) (delay * config.getMultiplier()),

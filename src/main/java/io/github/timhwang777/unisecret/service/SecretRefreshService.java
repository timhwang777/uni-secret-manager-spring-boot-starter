...
public class SecretRefreshService {
    ...
    /**
     * Configuration properties (for potential future use).
     */
    ...
    public SecretRefreshService(SecretCache cache, SecretManagerProperties properties) {
        this.cache = cache;
    ...
    public void refresh(String secretKey, String version, String field) {
        // Build the exact cache key that was used to store this secret
        String cacheKey = cache.buildKey(secretKey, version, field);

        // Remove from cache - next access will trigger fresh fetch
